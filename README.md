...
2. Before your first release, edit `app/package.json`: replace
   `YOUR_GITHUB_USERNAME` / `YOUR_REPO_NAME` in both the `repository` field
...
4. GitHub Actions picks up the tag, builds installers for all three
   platforms plus the firmware `.bin`, and attaches them all to a new
   Release automatically. Takes a few minutes — check the Actions tab.
